...
// divka.style.backgroundColor = 'grey';
// divka.style.color = 'blue';
// divka.style.fontSize = '20px'
// divka.innerHTML = '<h2>HEY YOU</h2>'
// document.body.append(divka)
// const clone = divka.cloneNode(true)
...
// const market = ['Main','Products','About us','Contacts'];
// const ulka = document.querySelector('.menu')
// const ulka = document.getElementsByClassName('menu')[0]
// for (const string of market) {
...
//     htmlDivElement.innerHTML = `Title - ${element.title} <br> MonthDuration - ${element.monthDuration }<hr>`
...
// for (const element of coursesAndDurationArray) {
//     let htmlDivElement = document.createElement('div');
//     htmlDivElement.setAttribute('class','item');
...
//     htmlDivElement.innerHTML = `NAME - ${simpson.name} <br> Surname - ${simpson.surname} <br> AGE - ${simpson.age} <br> ${simpson.info} <br>
...
//     photo.innerHTML = `<img src= ${simpson.photo} alt="Simson">`;
//     document.body.appendChild(htmlDivElement);
...
//     allElement.classList.add('member')
// }
...
//  останнє завдання не зробив до кінця

for (let i = 0; i < coursesArray.length; i++) {
    const element = coursesArray[i];
    ...
    titlle.innerHTML = element.title
    month.innerHTML = element.monthDuration
    hour.innerHTML = element.hourDuration
    for (let j = 0; j < coursesArray.modules.length; j++) {
        const module = coursesArray.modules[j];
        modul.innerHTML = `${}`

    }

    divElement.append(titlle,month,hour,modules)
